...
const stakeSheetIndex = file.indexOf('ref={stakeSheetRef}');
const scrollStart = file.indexOf('<ScrollView', stakeSheetIndex);
const scrollEnd = file.indexOf('</ScrollView>', scrollStart);
let scrollContent = file.substring(scrollStart, scrollEnd + '</ScrollView>'.length);

// remove the header row from stakeSheet (since we have the screen header)
scrollContent = scrollContent.replace(/<View style=\{\{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 \}\}>[\s\S]*?<\/TouchableOpacity>\s*<\/View>/, '').trim();

const rbSheetContent = file.substring(file.indexOf('<RBSheet\n        ref={confirmOverviewSheetRef}'), file.indexOf('</RBSheet>', file.indexOf('<RBSheet\n        ref={confirmOverviewSheetRef}')) + '</RBSheet>'.length);
